...
- Produces:
  ...
  - `SpanBuilder.BuildIntensity(IReadOnlyList<Bucket> buckets, long fromTs, long toTs, int bucketSeconds = 10)` → `Intensity`
...
```csharp
    ...
    private static string? ResolveName(Bucket bucket, IReadOnlyDictionary<long, string> paths)
    {
        ...
        return paths.TryGetValue(bucket.FgAppId.Value, out var path) ? Path.GetFileName(path) : null;
    }
```
...
public class GameSpanTests
{
    ...
    [Fact]
    public void MergesTwoOverlappingGamesIntoOneTrack()
    {
            ...
            if (covering is { } activeRun)
            {
                var focused = byTs.TryGetValue(ts, out var bucket)
                    ...
                    && folders.IsGame(fgPath)
                        ? Path.GetFileName(fgPath)
                        : null;

                slot = focused is not null
                    ? ("fg", focused)
                    : ("bg", Path.GetFileName(paths[activeRun.AppId]));
            }
...
Если тест `MergesTwoOverlappingGamesIntoOneTrack` падает из-за смены имени игры на границе — это ожидаемое поведение алгоритма «первый покрывающий run»: `gameRuns` отсортирован по `Started`, поэтому пока жив первый запуск, имя не меняется. Проверить порядок сортировки, а не ослаблять тест.
...
public sealed class ReportBuilder
{
    ...
    public string BuildJson(long nowTs, int historyDays = 90)
    {
        ...
        var payload = new
        {
            ...
            intensity = SpanBuilder.BuildIntensity(buckets, from, to, BucketSeconds)
        };
...
- Produces: `PcLogger.Core.Recording.RecorderService(ISystemProbe system, IProcessProbe processes, Db db, int bucketSeconds = 10)` с методами `void Tick(long unixSeconds)` и `void Stop(long unixSeconds)`
