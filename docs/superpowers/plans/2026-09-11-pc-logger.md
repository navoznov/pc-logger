...
**Before starting — требования к оформлению.** Палитра шаблона уже задана переменными
CSS со светлой и тёмной темами, легендой и подписанными осями; менять её не нужно. Но два
правила обязательны и проверяются на ревью:

1. **Вердикт по режиму не должен различаться только цветом.** `--ok` зелёный, `--over`
   красный — это самая частая форма цветовой слепоты, и дорожка «Режим» отвечает ровно на
   тот вопрос, ради которого открывают отчёт. Блокам с превышением нужен второй признак,
   не зависящий от цвета: диагональная штриховка поверх заливки.
2. **Никаких внешних ресурсов.** Отчёт открывается с диска, часто без сети: ни шрифтов с
   CDN, ни картинок по URL, ни внешних стилей. Всё встроено в файл.
...
function paint(trackId, boxes, color) {
  ...
  boxes.forEach(function (box) {
    ...
    // Longhand, not the `background` shorthand: the shorthand resets background-image
    // and would silently discard the .seg.over hatch that keeps the verdict readable
    // without colour.
